...
- ルート `README.md` と `workspace/v0.1.0` の設計文書のみが存在する。
- `pyproject.toml`、package skeleton、tests、CI workflow は未作成。
- 現在ブランチは `main`。
- 作業ツリーは clean。
...
- Bootstrap Python package and CI
- Define v1 sidecar schema
- Implement sanitizer module
- Implement local SQLite event store
- Implement sidecar health/events/suggest

P1:

- Add deterministic ranking fallback
- Migrate MyCodeBranchDesk exporter
- Add dataset split and stats
- Add evaluation metrics
- Add Anvil shadow-mode contract fixtures

P2:

- Add PHOTON MLX adapter
- Add checkpoint load interface
- Add macOS MLX smoke workflow
- Add MCP / stdio adapter design note
...
- [ ] schema models 実装
- [ ] schema fixtures 追加
- [ ] Anvil WorkingMemory 相当 fixture 追加
...
- [ ] FastAPI server
- [ ] fail-open client
- [ ] synthetic event smoke
- [ ] no-model suggest smoke
...
- [ ] candidate extractor
- [ ] fallback ranker
- [ ] repeated action / missing evidence warning

### Phase 5: Model and eval

- [ ] MLX optional extra
- [ ] PHOTON adapter interface
- [ ] checkpoint I/O
- [ ] macOS smoke workflow
- [ ] offline eval runner
- [ ] Anvil shadow contract
...
- `develop` ブランチを作成してよいか。
- package manager は標準 `pip` / `pyproject.toml` で進めるか、`uv` を採用するか。
- Python version は docs 通り 3.12 で固定してよいか。
- MyCodeBranchDesk DB は local-only 入力として扱い、raw fixture は commit しない方針でよいか。
- `POST /v1/summarize` と `POST /v1/evaluate` は M2 では stub、M6 で実体化する方針でよいか。
